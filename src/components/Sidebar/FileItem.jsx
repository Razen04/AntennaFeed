...
const FileItem = ({ item, handleFileClick, handleFileDelete, getFavicon }) => {
    return (
        <div className='files py-2 overflow-scroll'>
            <div className='flex relative'>
                <img src={item.selected ? markerLogo : null} alt="" />
                <div className='ml-[1rem] flex justify-between items-center transition-all hover:bg-violet-400 p-1 cursor-pointer w-full'>
                    <div className='flex items-center' onClick={() => handleFileClick(item.xmlurl)}>
                        <img src={getFavicon(item.xmlurl) || rssLogo} alt="" className='w-5' />
